...
test.describe('Authentication Integration Tests', () => {
  test.describe('Login Flow', () => {
    test('successful login with valid admin credentials', async ({ page }) => {
      ...
      // Dashboard should show Pierre branding
      await expect(page.getByText('Pierre', { exact: true }).first()).toBeVisible();
    });
  ...
  test.describe('Session Management', () => {
    test('dashboard displays real user data from server', async ({ page }) => {
      ...
      expect(statsVisible || await page.getByText('Pierre', { exact: true }).first().isVisible()).toBe(true);
    });

    // Skip in single-process mode (containers) - multiple contexts not supported
    const skipMultiContext = process.env.PLAYWRIGHT_SINGLE_PROCESS === 'true';

    test('multiple users can have separate sessions', async ({ browser }) => {
      test.skip(skipMultiContext, 'Multi-context not supported in single-process mode');

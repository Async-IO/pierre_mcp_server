...
test.describe('Dashboard Integration Tests', () => {
  ...
  test.describe('Navigation', () => {
    test('can navigate between dashboard tabs', async ({ page }) => {
      // Admin dashboard has these tabs in the sidebar
      const tabs = ['Overview', 'Connections', 'Tools'];

      for (const tab of tabs) {
        // Dashboard uses sidebar nav buttons with span text
        ...
        if (isVisible) {
          ...
          if (headerVisible) {
            const headerContent = await headerText.textContent().catch(() => '');
            expect(headerContent).toContain(tab);
          }
        }
      }
    });
    ...
    test('sidebar navigation works correctly', async ({ page }) => {
      // Dashboard has an aside element for the sidebar
      const sidebarVisible = await page.locator('aside').first().isVisible();
      expect(sidebarVisible).toBe(true);

      // Should show Pierre logo/branding
      const hasLogo = await page.locator('text=Pierre').first().isVisible();
  ...
  test.describe('Error Handling', () => {
    test('dashboard handles API errors gracefully', async ({ page }) => {
      ...
      // Check if page handles error gracefully (doesn't crash, shows some content)
      ...
      // Test passes if page shows error message, shows "Pierre" branding, or has meaningful content
      expect(hasError || pageLoaded || hasAnyContent).toBe(true);
    });
  });
});

...
use crate::admin::models::ValidatedAdminToken;
...
impl AdminConfigState {
    ...
    #[must_use]
    pub const fn new(service: Arc<AdminConfigService>) -> Self {
        Self { service }
    }
}
...
pub async fn update_category_config(
    ...
    let catalog = state
        ...
        .get_catalog(query.tenant_id.as_deref())
        .await?;
    let category_keys: std::collections::HashSet<String> = catalog
        .categories
        .iter()
    ...
    let filtered_params: std::collections::HashMap<String, serde_json::Value> = request
        .parameters
        .into_iter()
...
pub fn admin_config_router(state: Arc<AdminConfigState>) -> axum::Router {
    ...
    axum::Router::new()
        ...
        .route("/history", get(get_audit_log))
        .with_state(state)
}

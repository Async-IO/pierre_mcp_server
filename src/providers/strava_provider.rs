...
struct StravaStatsResponse {
    biggest_ride_distance: Option<f32>,
    biggest_climb_elevation_gain: Option<f32>,
    recent_ride_totals: Option<StravaTotals>,
    recent_run_totals: Option<StravaTotals>,
    ytd_ride_totals: Option<StravaTotals>,
    ytd_run_totals: Option<StravaTotals>,
...
impl StravaProvider {
        ...
        T: for<'de> Deserialize<'de>,
    {
        ...
        let url = format!("{}/{}", self.config.api_base_url, endpoint.trim_start_matches('/'));
...
impl FitnessProvider for StravaProvider {
    ...
    async fn get_activities(&self, limit: Option<usize>, offset: Option<usize>) -> Result<Vec<Activity>> {
        ...
        let endpoint = format!("athlete/activities?per_page={}&page={}", limit, page);
        ...
        for activity in strava_activities {
            ...
            activities.push(Activity {
                ...
                start_date,
                distance_meters: activity.distance.map(|d| d as f64),
                duration_seconds: activity.elapsed_time.unwrap_or(0) as u64,
                elevation_gain: activity.total_elevation_gain.map(|e| e as f64),
                average_speed: activity.average_speed.map(|s| s as f64),
                max_speed: activity.max_speed.map(|s| s as f64),
                average_heart_rate: activity.average_heartrate.map(|hr| hr as u32),
                max_heart_rate: activity.max_heartrate.map(|hr| hr as u32),
                average_cadence: activity.average_cadence.map(|c| c as u32),
                average_power: activity.average_watts.map(|p| p as u32),
                max_power: activity.max_watts.map(|p| p as u32),
                ...
                suffer_score: activity.suffer_score.map(|s| s as u32),
    ...
    async fn get_activity(&self, id: &str) -> Result<Activity> {
        ...
        Ok(Activity {
            ...
            start_date,
            distance_meters: strava_activity.distance.map(|d| d as f64),
            duration_seconds: strava_activity.elapsed_time.unwrap_or(0) as u64,
            elevation_gain: strava_activity.total_elevation_gain.map(|e| e as f64),
            average_speed: strava_activity.average_speed.map(|s| s as f64),
            max_speed: strava_activity.max_speed.map(|s| s as f64),
            average_heart_rate: strava_activity.average_heartrate.map(|hr| hr as u32),
            max_heart_rate: strava_activity.max_heartrate.map(|hr| hr as u32),
            average_cadence: strava_activity.average_cadence.map(|c| c as u32),
            average_power: strava_activity.average_watts.map(|p| p as u32),
            max_power: strava_activity.max_watts.map(|p| p as u32),
            calories: None, // Strava doesn't provide calories in basic activity data
            steps: None,
            heart_rate_zones: None,
            normalized_power: None,
            ...
            training_stress_score: None,
            intensity_factor: None,
            suffer_score: strava_activity.suffer_score.map(|s| s as u32),
            time_series_data: None,
            start_latitude: None,
            start_longitude: None,
            city: None,
    ...
    async fn get_stats(&self) -> Result<Stats> {
        ...
        Ok(Stats {
            total_activities: (stats.all_ride_totals.as_ref().map(|t| t.count).unwrap_or(0) +
                             stats.all_run_totals.as_ref().map(|t| t.count).unwrap_or(0)) as u64,
            total_distance: (stats.all_ride_totals.as_ref().map(|t| t.distance).unwrap_or(0.0) +
                                  stats.all_run_totals.as_ref().map(|t| t.distance).unwrap_or(0.0)) as f64,
            total_duration: (stats.all_ride_totals.as_ref().map(|t| t.moving_time).unwrap_or(0) +
                                   stats.all_run_totals.as_ref().map(|t| t.moving_time).unwrap_or(0)) as u64,
            total_elevation_gain: (stats.all_ride_totals.as_ref().map(|t| t.elevation_gain).unwrap_or(0.0) +
                                        stats.all_run_totals.as_ref().map(|t| t.elevation_gain).unwrap_or(0.0)) as f64,
        })
    }

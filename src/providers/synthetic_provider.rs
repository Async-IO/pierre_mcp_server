...
impl SyntheticProvider {
    ...
    fn generate_activities_from_rng(
        ...
        for i in 0..count {
            ...
            let start_date = base_date - Duration::days(days_ago) - Duration::hours(24 - hour_offset);
            ...
            let distance_meters = match sport_type {
                SportType::Run => Some(rng.gen_range(3000.0..21000.0)),
                SportType::Ride => Some(rng.gen_range(10000.0..100000.0)),
                SportType::Swim => Some(rng.gen_range(500.0..4000.0)),
                SportType::Walk => Some(rng.gen_range(2000.0..8000.0)),
                SportType::Hike => Some(rng.gen_range(5000.0..25000.0)),
                SportType::Workout => None,
                _ => None,
            };
    ...
    fn generate_sleep_sessions_from_rng(
        ...
        for i in 0..nights {
            ...
            let sleep_duration_hours = rng.gen_range(6..9);
            let sleep_start = end_date - Duration::hours(bedtime_offset);
            let sleep_end = sleep_start + Duration::hours(sleep_duration_hours);
            ...
            let total_sleep_time = (sleep_duration_hours * 60) as u32 - rng.gen_range(10..40);

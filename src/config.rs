...
"#;
        ...
        assert_eq!(
            ...
            Some("file_access_token".to_string())
        );
    ...
    fn test_config_load_from_env_vars() {
        ...
        let temp_dir = TempDir::new().expect("Failed to create temp dir");
        let nonexistent_config = temp_dir
            .path()
            .join("definitely_nonexistent_config_12345.toml");
        ...
        assert!(
            config.providers.contains_key("strava"),
        ...
        assert_eq!(
            strava_config.access_token,
            Some("test_env_access_token_unique_abcdef".to_string())
        );
        assert_eq!(
            strava_config.refresh_token,
            Some("test_env_refresh_token_unique_ghijkl".to_string())
        );
    ...
    fn test_config_load_missing_env_vars() {
        ...
        let config = Config::load(Some(config_path)).expect("Failed to load empty config");
    ...
    fn test_config_save() {
        ...
        let loaded_config =
            Config::load(Some(config_path_str)).expect("Failed to load saved config");
    ...
    fn test_config_save_creates_directory() {
        ...
        let loaded_config =
            Config::load(Some(nested_path_str)).expect("Failed to load saved config");
        assert_eq!(loaded_config.providers.len(), config.providers.len());
    }
}

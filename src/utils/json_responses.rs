...
mod tests {
    ...
    fn test_simple_error() {
        let result = simple_error("Test error message");
        let expected = json!({"error": "Test error message"});
    ...
    fn test_simple_success() {
        ...
        let expected = json!({"success": true, "message": "Operation completed"});
        assert_eq!(result, expected);
    }

    #[test]
    fn test_detailed_error() {
        let result = detailed_error("invalid_input", "The provided input is invalid");
        let expected = json!({
            "success": false,
            "error": "invalid_input",
            "error_description": "The provided input is invalid"
        });
        assert_eq!(result, expected);
    }

    #[test]
    fn test_oauth_error_with_provider() {
        let result = oauth_error(
    ...
    #[test]
    fn test_oauth_error_without_provider() {
        let result = oauth_error("authorization_failed", "Invalid credentials", None);
        let expected = json!({
            "error": "authorization_failed",
            "details": "Invalid credentials"
        });
    ...
    fn test_provider_connection_error() {
        let result = provider_connection_error("Strava", "No token found");
        assert!(result["error"].as_str().unwrap().contains("Strava"));
        assert!(result["note"].as_str().unwrap().contains("OAuth flow"));
        assert_eq!(result["is_real_data"], false);
    }

    #[test]
    fn test_not_found_error() {
        let result = not_found_error("user");
        let expected = json!({
            "error": "not_found",
    ...
    fn test_rate_limit_error() {
        let result = rate_limit_error(100, Some("2025-01-01T00:00:00Z"));
        let expected = json!({
            "error": "rate_limit_exceeded",
            "message": "API rate limit exceeded",
            "limit": 100,
    ...
    #[test]
    fn test_activity_not_found_error() {
        let result = activity_not_found_error("12345", Some("Strava"));
        let expected = json!({
    ...
    #[test]
    fn test_detailed_success() {
        let data = json!({"count": 5, "items": []});
        let result = detailed_success("Data retrieved successfully", &data);
        let expected = json!({
            "success": true,

...
async fn test_request_id_uniqueness() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _ in 0..5 {
        ...
        if let Some(request_id) = response.headers().get("x-request-id") {
            request_ids.push(request_id.to_str()?.to_string());
        }
    }
...
fn test_request_id_display_trait() {
    let request_id = RequestId("test-request-id-12345".to_string());

    // Test Display trait
    let display_output = format!("{}", request_id);
...
async fn test_request_id_uuid_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(request_id_header) = response.headers().get("x-request-id") {
        ...
        panic!("Request ID header not found");
    }
...
#[tokio::test]
async fn test_jwt_secret_not_logged() {
...
#[tokio::test]
async fn test_database_operation_instrumentation() {
    use pierre_mcp_server::database_plugins::{factory::Database, DatabaseProvider};
...
async fn test_provider_instrumentation_integration() {
    ...
    assert!(true, "Provider instrumentation compiles correctly");
}
...
async fn test_request_id_concurrency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _ in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let response = app_clone.oneshot(request).await.expect("Request failed");

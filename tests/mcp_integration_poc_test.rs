...
use anyhow::Result;
use integration::{IntegrationTestServer, McpTestClient, TestFixtures};
use serde_json::json;
...
async fn test_mcp_initialize() -> Result<()> {
    ...
    assert!(!result.server_info.name.is_empty(), "Server name should be set");
    assert!(!result.server_info.version.is_empty(), "Server version should be set");

    println!("✅ MCP initialize: protocol={}, server={} v{}",
...
async fn test_mcp_tools_list() -> Result<()> {
    ...
    for expected in expected_tools {
        assert!(
            ...
            "Tool '{}' should be available. Found: {:?}",
            expected,
            tool_names
        );
    }
...
async fn test_mcp_tool_call_connection_status() -> Result<()> {
    ...
    let text = result.content[0].text.as_ref().expect("Should have text content");
...
async fn test_mcp_invalid_tool() -> Result<()> {
    ...
    assert!(
        error_msg.contains("not found") || error_msg.contains("unknown") || error_msg.contains("MCP error"),
...
async fn test_mcp_sequential_requests() -> Result<()> {
    ...
    for i in 0..5 {
        let result = client.list_tools().await?;
        assert!(!result.tools.is_empty(), "Request {} should return tools", i);
    }

...
async fn test_activity_analysis_through_universal_tools() -> Result<()> {
    ...
    let _tool_name = "analyze_activity";
...
async fn test_goal_tracking_integration() -> Result<()> {
    ...
    let goal_data = serde_json::json!({
        "type": "distance",
        "target_value": 100000.0, // 100km total distance
        "current_value": 0.0,
        "time_period": "monthly",
        "sport_type": "run",
    ...
    assert_eq!(
        updated_goal["current_value"].as_f64().unwrap(),
        total_distance
    );
    assert_eq!(updated_goal["progress_percentage"].as_f64().unwrap(), 23.0); // 23% of 100km

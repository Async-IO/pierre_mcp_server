...
fn test_macro_targets_from_calories() {
    ...
    assert!(
        (targets.protein_g.expect("protein should be set") - 37.5).abs() < 0.1,
        "Expected ~37.5g protein"
    );
    assert!(
        (targets.carbs_g.expect("carbs should be set") - 56.25).abs() < 0.1,
        "Expected ~56.25g carbs"
    );
    assert!(
        (targets.fat_g.expect("fat should be set") - 13.89).abs() < 0.1,
        "Expected ~13.89g fat"
    );
}
...
fn test_weight_conversions() {
    // Grams (identity)
    let grams_result = convert_to_grams("anything", 100.0, IngredientUnit::Grams)
        .expect("Grams conversion should succeed");
    assert!(
    ...
    // Kilograms
    let kg_result = convert_to_grams("anything", 1.5, IngredientUnit::Kilograms)
        .expect("Kilograms conversion should succeed");
    assert!(
    ...
    // Ounces
    let oz_result = convert_to_grams("anything", 1.0, IngredientUnit::Ounces)
        .expect("Ounces conversion should succeed");
    assert!(
    ...
    // Pounds
    let lb_result = convert_to_grams("anything", 1.0, IngredientUnit::Pounds)
        .expect("Pounds conversion should succeed");
    assert!(
...
fn test_volume_conversions() {
    ...
    let rice_grams =
        convert_to_grams("rice", 1.0, IngredientUnit::Cups).expect("Rice conversion should work");
    assert!(
    ...
    let oil_grams = convert_to_grams("olive oil", 1.0, IngredientUnit::Tablespoons)
        .expect("Oil conversion should work");
    assert!(
    ...
    let honey_grams = convert_to_grams("honey", 1.0, IngredientUnit::Teaspoons)
        .expect("Honey conversion should work");
    assert!(
...
fn test_count_conversions() {
    // 2 eggs = 2 * 50g = 100g
    let egg_grams =
        convert_to_grams("egg", 2.0, IngredientUnit::Pieces).expect("Egg conversion should work");
    assert!((egg_grams - 100.0).abs() < 0.1, "2 eggs should be ~100g");

    // 1 banana = 120g
    let banana_grams = convert_to_grams("banana", 1.0, IngredientUnit::Pieces)
        .expect("Banana conversion should work");
    assert!(
    ...
    let garlic_grams = convert_to_grams("garlic clove", 3.0, IngredientUnit::Pieces)
        .expect("Garlic conversion should work");
    assert!(
...
fn test_alias_matching() {
    ...
    let grams =
        convert_to_grams("chicken", 1.0, IngredientUnit::Cups).expect("Alias matching should work");
    assert!(grams > 0.0, "Should find chicken via partial match");

    // "rolled oats" should match "oats"
    let grams =
        convert_to_grams("rolled oats", 1.0, IngredientUnit::Cups).expect("Oats alias should work");
    assert!(grams > 0.0, "Should find oats via alias");
}

#[test]
fn test_case_insensitivity() {
    let lower = convert_to_grams("rice", 1.0, IngredientUnit::Cups).expect("lowercase should work");
    let upper = convert_to_grams("RICE", 1.0, IngredientUnit::Cups).expect("uppercase should work");
    let mixed = convert_to_grams("Rice", 1.0, IngredientUnit::Cups).expect("mixedcase should work");

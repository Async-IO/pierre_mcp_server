...
async fn test_error_invalid_jwt() -> Result<()> {
    ...
    assert!(
        error.contains("JWT") || error.contains("auth") || error.contains("invalid") || error.contains("malformed"),
...
async fn test_error_unknown_provider() -> Result<()> {
    ...
    let result = client
        .call_tool_raw("get_activities", json!({"provider": "nonexistent_provider_xyz"}))
        .await;
    ...
    match result {
        Ok(r) => {
            ...
            println!("✅ Unknown provider handled: {}", &text[..text.len().min(100)]);
        }
        Err(e) => {
...
async fn test_error_provider_not_connected() -> Result<()> {
    ...
    match result {
        Ok(r) if !r.is_error => {
            ...
            assert!(
                text.contains("connect") || text.contains("oauth") || text.contains("http") || text.contains("not connected"),
                "Should mention how to connect: {}",
                text
            );
...
async fn test_error_missing_required_params() -> Result<()> {
    ...
    match result {
        ...
        Ok(r) => {
            let empty = String::new();
            let text = r.content[0].text.as_ref().unwrap_or(&empty);
            println!("✅ Missing params handled: {}", &text[..text.len().min(100)]);
        }
        Err(e) => {
...
async fn test_error_invalid_jsonrpc_version() -> Result<()> {
    ...
    println!("✅ Invalid JSON-RPC version returned status: {}", response.status());
    Ok(())
}
...
async fn test_error_rapid_requests() -> Result<()> {
    ...
    assert!(
        ...
        "At least half of rapid requests should succeed: {} of {}",
        success_count,
        total_requests
    );

    println!("✅ Rapid requests: {}/{} succeeded", success_count, total_requests);
    Ok(())
}
...
async fn test_error_recovery_after_failure() -> Result<()> {
    ...
    let error_result = client
        .call_tool_raw("nonexistent_tool", json!({}))
        .await;
    ...
    assert!(success_result.is_ok(), "Should succeed after error recovery");

...
impl McpTestClient {
    ...
    pub async fn send_request_for_result(&self, method: &str, params: Option<Value>) -> Result<Value> {
    ...
    pub async fn initialize(&self) -> Result<InitializeResult> {
        ...
        let result = self.send_request_for_result("initialize", Some(params)).await?;
    ...
    pub async fn call_tool<T: Serialize>(&self, name: &str, arguments: &T) -> Result<ToolCallResult> {
        let params = json!({
            "name": name,
            "arguments": serde_json::to_value(arguments)?
        });

        let result = self.send_request_for_result("tools/call", Some(params)).await?;
        Ok(serde_json::from_value(result)?)
    }

...
async fn test_workflow_new_user_onboarding() -> Result<()> {
    ...
    let status = call_tool_lenient(&client, "get_connection_status", json!({"provider": "strava"})).await;
    println!("  ✓ Step 3: get_connection_status: {}", status);

    // Step 4: Get activities (will fail without connected provider - that's expected)
    let activities = call_tool_lenient(&client, "get_activities", json!({"provider": "synthetic", "limit": 10})).await;
    println!("  ✓ Step 4: get_activities: {}", activities);

    // Step 5: Get fitness score
    let score = call_tool_lenient(&client, "calculate_fitness_score", json!({"provider": "synthetic"})).await;
...
async fn test_workflow_training_analysis() -> Result<()> {
    ...
    let activities = call_tool_lenient(&client, "get_activities", json!({"provider": "synthetic", "limit": 30})).await;
    println!("  ✓ Step 1: get_activities: {}", activities);

    // Step 2: Analyze training load
    let load = call_tool_lenient(&client, "analyze_training_load", json!({"provider": "synthetic", "days": 14})).await;
    println!("  ✓ Step 2: analyze_training_load: {}", load);

    // Step 3: Get performance trends
    let trends = call_tool_lenient(&client, "analyze_performance_trends", json!({"provider": "synthetic", "days": 30})).await;
    println!("  ✓ Step 3: analyze_performance_trends: {}", trends);

    // Step 4: Get personalized recommendations
    let recommendations = call_tool_lenient(&client, "generate_recommendations", json!({"provider": "synthetic"})).await;
    println!("  ✓ Step 4: generate_recommendations: {}", recommendations);

    // Step 5: Set a new goal
    let goal = call_tool_lenient(&client, "set_goal", json!({
...
async fn test_workflow_recovery_check() -> Result<()> {
    ...
    let sleep = call_tool_lenient(&client, "analyze_sleep_quality", json!({"provider": "synthetic", "days": 7})).await;
    println!("  ✓ Step 1: analyze_sleep_quality: {}", sleep);

    // Step 2: Calculate recovery score
    let recovery = call_tool_lenient(&client, "calculate_recovery_score", json!({"provider": "synthetic"})).await;
    println!("  ✓ Step 2: calculate_recovery_score: {}", recovery);

    // Step 3: Get rest day suggestion
    let rest_day = call_tool_lenient(&client, "suggest_rest_day", json!({"provider": "synthetic"})).await;
    println!("  ✓ Step 3: suggest_rest_day: {}", rest_day);

    // Step 4: Get training load analysis
    let load = call_tool_lenient(&client, "analyze_training_load", json!({"provider": "synthetic", "days": 7})).await;
...
async fn test_workflow_nutrition_planning() -> Result<()> {
    ...
    let nutrition = call_tool_lenient(&client, "calculate_daily_nutrition", json!({
        "weight_kg": 70.0,
        "height_cm": 175.0,
        "age": 30,
        "gender": "male",
    ...
    let timing = call_tool_lenient(&client, "get_nutrient_timing", json!({
    ...
    let foods = call_tool_lenient(&client, "search_food", json!({"query": "chicken breast", "limit": 3})).await;
    println!("  ✓ Step 3: search_food: {}", foods);

    // Step 4: Get recipe constraints
    let constraints = call_tool_lenient(&client, "get_recipe_constraints", json!({"training_phase": "build"})).await;
...
async fn test_workflow_goal_management() -> Result<()> {
    ...
    let suggestions = call_tool_lenient(&client, "suggest_goals", json!({"provider": "synthetic"})).await;
    println!("  ✓ Step 1: suggest_goals: {}", suggestions);

    // Step 2: Set a specific goal
    let new_goal = call_tool_lenient(&client, "set_goal", json!({
    ...
    let feasibility = call_tool_lenient(&client, "analyze_goal_feasibility", json!({
        "provider": "synthetic",
        "goal_type": "distance",
        "target_value": 200.0
    })).await;
...
async fn test_workflow_configuration() -> Result<()> {
    ...
    let zones = call_tool_lenient(&client, "calculate_personalized_zones", json!({"vo2_max": 55.0})).await;
    println!("  ✓ Step 4: calculate_personalized_zones: {}", zones);

    // Step 5: Set fitness config
    let set_config = call_tool_lenient(&client, "set_fitness_config", json!({
...
async fn test_workflow_recipe_management() -> Result<()> {
    ...
    let constraints = call_tool_lenient(&client, "get_recipe_constraints", json!({"training_phase": "recovery"})).await;
    println!("  ✓ Step 1: get_recipe_constraints: {}", constraints);

    // Step 2: Save a new recipe
    let save_result = call_tool_lenient(&client, "save_recipe", json!({
    ...
    let search_result = call_tool_lenient(&client, "search_recipes", json!({"query": "protein"})).await;
...
async fn test_workflow_provider_comparison() -> Result<()> {
    ...
    let strava = call_tool_lenient(&client, "get_connection_status", json!({"provider": "strava"})).await;
    println!("  ✓ Strava status: {}", strava);

    let fitbit = call_tool_lenient(&client, "get_connection_status", json!({"provider": "fitbit"})).await;
    println!("  ✓ Fitbit status: {}", fitbit);

    let synthetic = call_tool_lenient(&client, "get_connection_status", json!({"provider": "synthetic"})).await;
    println!("  ✓ Synthetic status: {}", synthetic);

    // Get activities from synthetic provider
    let activities = call_tool_lenient(&client, "get_activities", json!({"provider": "synthetic", "limit": 5})).await;
...
async fn test_workflow_daily_routine() -> Result<()> {
    ...
    let sleep = call_tool_lenient(&client, "analyze_sleep_quality", json!({"provider": "synthetic", "days": 1})).await;
    ...
    let rest = call_tool_lenient(&client, "suggest_rest_day", json!({"provider": "synthetic"})).await;
